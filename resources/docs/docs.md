...
The `home-routes` handler in the `mortgag-calculator.routes.home` namespace
...
The `home-page` function will in turn call the `mortgag-calculator.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `mortgag-calculator.handler` namespace:
...
Request middleware functions are located under the `mortgag-calculator.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `mortgag-calculator.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `mortgag-calculator.db.core` in some other namespace.
